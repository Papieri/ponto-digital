...
describe("detectPeriod", () => {
  // PORTABILIDADE: no arquivo original estas asserções liam Date
  // (periodStart.getUTCDate() === 2, getUTCMonth() === 1). O parser em produção
  // devolve string UTC 'YYYY-MM-DD HH:MM:SS' desde a mudança para gravação sem
  // conversão de fuso. A asserção é a mesma — 02/02/2026 — na forma atual.
  it("should detect period from records", () => {
    const records = parseTxtContent(SAMPLE_CONTENT);
    const { periodStart, periodEnd } = detectPeriod(records);
    expect(periodStart).toBe("2026-02-02 00:00:00");
    expect(periodEnd).toBe("2026-02-02 23:59:59");
  });

  // PORTABILIDADE: original exigia toBeInstanceOf(Date). Mesma intenção —
  // "devolve um instante válido" — expressa no formato de string atual.
...
// PORTABILIDADE: recalcFromTimestamps recebe as strings 'YYYY-MM-DD HH:MM:SS'
// lidas do banco (é assim que routers.ts a chama). O arquivo original montava
// objetos Date. Os minutos esperados e as asserções continuam idênticos.
